...
export const getUpgradeRecommendations = (device: Device): string[] => {
  ...
  if (!device.TPMVersion || device.TPMVersion === 'None') {
    recommendations.push(
      'Enable TPM 2.0 in BIOS/UEFI settings or install a TPM module if supported.'
    );
  ...
  if (device.TotalRAMGB < 4) {
    recommendations.push(
      `Upgrade RAM from ${device.TotalRAMGB}GB to at least 4GB (recommended: 8GB or more).`
    );
  }

  if (device.FreeStorageGB < 64) {
    recommendations.push(
      `Free up storage space or add more storage. Need at least 64GB free (currently: ${device.FreeStorageGB.toFixed(1)}GB).`
    );
  }
  ...
  if (windowsVersion.includes('Windows 7') || windowsVersion.includes('Windows 8')) {
    recommendations.push(
      'Current OS is very old. Consider upgrading to Windows 10 first, then Windows 11.'
    );
  }
